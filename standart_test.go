...
func TestStandart(t *testing.T) {
	...
	t.Run("Marshal 0 user", func(t_ *testing.T) {
		...
		U := InitUsersStruct(0)
		got, err := MarshalStandart(U)
		if err != nil {
			t_.Errorf(RED_BG + "Error marshal standart: " + err.Error() + NO_COLOR)
			return
		}
		err = byteComparator(expected, got)
		if err != nil {
			t_.Errorf(RED_BG + "Marshal standart missmatch: " + err.Error() + NO_COLOR)
			return
		}
	...
	t.Run("Unmarshal 0 user", func(t_ *testing.T) {
		...
		Ugot, err := UnmarshalStandart([]byte(xmlSrc))
		if err != nil {
			t_.Errorf(RED_BG + "Error unmarshal standart: " + err.Error() + NO_COLOR)
			return
		}
		err = UserComparator(Uexpected, Ugot)
		if err != nil {
			t_.Errorf(RED_BG + "Unmarshal standart missmatch: " + err.Error() + NO_COLOR)
			return
		}
	...
	t.Run("Marshal unmarshal 0 users", func(t_ *testing.T) {
		Uexpected := InitUsersStruct(0)
		xmlMid, err := MarshalStandart(Uexpected)
		if err != nil {
			t_.Errorf(RED_BG + "Error marshal standart: " + err.Error() + NO_COLOR)
			return
		}
		Ugot, err := UnmarshalStandart([]byte(xmlMid))
		if err != nil {
			t_.Errorf(RED_BG + "Error unmarshal standart: " + err.Error() + NO_COLOR)
			return
		}
		xmlMid, err = MarshalStandart(Ugot)
		if err != nil {
			t_.Errorf(RED_BG + "Error marshal standart: " + err.Error() + NO_COLOR)
			return
		}
